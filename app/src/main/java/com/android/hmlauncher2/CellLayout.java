...
import com.android.hmlauncher2.FolderIcon.FolderRingAnimator;
...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        Log.d(TAG, "metrics.widthPixels:" + metrics.widthPixels + " and metrics.heightPixels:" + metrics.heightPixels);
        Log.d(TAG, "mWidthGap:" + mWidthGap + " and mHeightGap:" + mHeightGap);

        int statusBarHeight = getStatusBarHeight();
        int naviBarHeight = getNavigationBarHeight(context);
        Log.d(TAG, "statusBarHeight:" + statusBarHeight + " naviBarHeight: " + naviBarHeight);

        mCellWidth = (metrics.widthPixels - mWidthGap * (mCountX + 1) - 15) / mCountX;
        mCellHeight = (metrics.heightPixels -statusBarHeight - naviBarHeight - mHeightGap * (mCountY + 1)- 16) / mCountY;

        Log.d(TAG, "mCellWidth:" + mCellWidth + " mCellHeight: " + mCellHeight +
                ", with mCountX:" + mCountX + " mCountY:" + mCountY);



        LauncherModel.updateLayoutCellSize(mCellWidth, mCellHeight);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mCellWidth = (newWidth - mWidthGap * (mCountX + 1)) / mCountX;
        mCellHeight = (newHeight - mHeightGap * (mCountY + 1)) / mCountY;
        LauncherModel.updateLayoutCellSize(mCellWidth, mCellHeight);
    ...
    private int[] findNearestArea(int cellX, int cellY, int spanX, int spanY, int[] direction,
        ...
        // Keep track of best-scoring drop area
        Log.d("DragLayer", "findNearestArea() is called with cellX:" + cellX + " cellY:" + cellY
            ...
            for (int x = 0; x < countX - (spanX - 1); x++) {
                ...
                for (int i = 0; i < spanX; i++) {
                    for (int j = 0; j < spanY; j++) {
                        Log.d("DragLayer", "findNearestArea(): y:" + y + " x:" + x + " i:"+ i + " j:"+ j);
                        if (occupied[x + i][y + j] && (blockOccupied == null || blockOccupied[i][j])) {
                            int m = x + i;
                            int n = y + j;
                            Log.d("DragLayer", "occupied[" + m + "][" + n + "] is " + occupied[m][n]);
                            if (blockOccupied == null) {
                                Log.d("DragLayer", "blockOccupied["
                                        + i + "][" + j + "] is + null \n");
                            } else {
                                Log.d("DragLayer", "blockOccupied[" + i + "][" + j + "]:" + blockOccupied[i][j] + "\n");
                            }
                            continue inner;
                        } else {
                            int m = x + i;
                            ...
                            Log.d("DragLayer", "occupied[" + m + "][" + n + "] is " + occupied[m][n]);
                            if (blockOccupied == null) {
                                Log.d("DragLayer", "blockOccupied[" + i + "][" + j + "] is + null \n");
                            } else {
                                Log.d("DragLayer", "blockOccupied[" + i + "][" + j + "]:" + blockOccupied[i][j] + "\n");
                            }
                        }
                ...
                Log.d("DragLayer", "findNearestArea(): y:" + y + " x:" + x );
    ...
    private boolean addViewToTempLocation(View v, Rect rectOccupiedByPotentialDrop,
        ...
        Log.d("DragLayer", "addViewToTempLocation() is called with the view:" + v.toString()
        + " rectOccupiedByPotentialDrop:" + rectOccupiedByPotentialDrop.toString());
    ...
    private boolean swapViews(View view, View dragView, ItemConfiguration currentState) {
        Log.d("DragLayer", "swapViews() called with view:" + view.toString()
                + " and dragView:" +dragView.toString());

        //Find AppCardInfo for two views
        LauncherAppCardInfo dragedCard = null;
        LauncherAppCardInfo targetedCard = null;
        CellAndSpan dragCellAndSpan = null;
        CellAndSpan targetedCellAndSpan = null;
        LayoutParams dragLp = null;
        LayoutParams targetLp = null;
        for (View child: currentState.map.keySet()) {
            Log.d("DragLayer", "swapViews(): child:" + child.toString() + " with CellAndSpan:"
                    + currentState.map.get(child).toString());

            if (child == dragView) {
                dragedCard = (LauncherAppCardInfo) child.getTag();
                dragCellAndSpan = currentState.map.get(child);
                dragLp = (LayoutParams) child.getLayoutParams();
            } else if (child == view) {
                targetedCard = (LauncherAppCardInfo) child.getTag();
                targetedCellAndSpan = currentState.map.get(child);
                targetLp = (LayoutParams) child.getLayoutParams();
            }
        }

        if ((dragedCard == null) || (targetedCard == null)) return false;

        markCellsForView(dragLp.cellX, dragLp.cellY,
                dragLp.cellHSpan, dragLp.cellVSpan, mTmpOccupied, true);


        markCellsForView(targetLp.cellX, targetLp.cellY,
                targetLp.cellHSpan, targetLp.cellVSpan, mTmpOccupied, true);

//
//        //switch the App Card Info
//        for (View child: currentState.map.keySet()) {
//            Log.d("DragLayer", "swapViews(): child again:" + child.toString() + " with CellAndSpan:"
//                    + currentState.map.get(child).toString());
//            if (child == dragView) {
//                targetedCard.cellX = dragCellAndSpan.x;
//                targetedCard.cellY = dragCellAndSpan.y;
//                targetedCard.spanX = dragCellAndSpan.spanX;
//                targetedCard.spanY = dragCellAndSpan.spanY;
//                Log.d("DragLayer", "swapViews(): set dragged view to the tag:" + dragedCard.toString() +
//                        " with cellX:" + targetedCard.cellX + " cellY:" + targetedCard.cellY + "spanX:"
//                        + targetedCard.spanX + "spanY:" + targetedCard.spanY);
//                child.setTag(targetedCard);
//                markCellsForView(targetedCard.cellX, targetedCard.cellY,
//                        targetedCard.spanX, targetedCard.spanY, mTmpOccupied, true);
//
//            } else if (child == view) {
//                dragedCard.cellX = targetedCellAndSpan.x;
//                dragedCard.cellY = targetedCellAndSpan.y;
//                dragedCard.spanX = targetedCellAndSpan.spanX;
//                dragedCard.spanY = targetedCellAndSpan.spanY;
//                Log.d("DragLayer", "swapViews(): set target view to the tag:" + dragedCard.toString() +
//                    " with cellX:" + dragedCard.cellX + " cellY:" + dragedCard.cellY + "spanX:"
//                    + dragedCard.spanX + "spanY:" + dragedCard.spanY);
//                child.setTag(dragedCard);
//                markCellsForView(dragedCard.cellX, dragedCard.cellY,
//                        dragedCard.spanX, dragedCard.spanY, mTmpOccupied, true);
//            }
//        }


        Log.d("DragLayer", "swapViews(): switch layout parameters!!!");
        dragView.setLayoutParams(targetLp);
        //((View)dragView.getTag(R.id.all_apps_cling)).setLayoutParams(targetLp);
        view.setLayoutParams(dragLp);

        requestLayout();
        return false; //On purpose
    }
    ...
    private boolean addViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
            int[] direction, View dragView, ItemConfiguration currentState) {
        Log.d("DragLayer", "addViewsToTempLocation() called with rectOccupiedByPotentialDrop:"
                + rectOccupiedByPotentialDrop.toString());
        ...
        for (View v: views) {
            Log.d("DragLayer", "addViewsToTempLocation(): view:" + v.toString());
            CellAndSpan c = currentState.map.get(v);
        ...
        Log.d("DragLayer", "addViewsToTempLocation(): boundingRect:"+ boundingRect.toString());
        ...
        for (View v: views) {
            ...
            //markCellsForView(c.x - left, c.y - top, c.spanX, c.spanY, blockOccupied, false);
        }
    ...
    private boolean rearrangementExists(int cellX, int cellY, int spanX, int spanY, int[] direction,
        ...
        Log.d("DragLayer", "rearrangementExists() is called with cellX:" + cellX + " cellY:" + cellY
                + "spanX:" + spanX + " spanY:" + spanY);
        View biggerView = null;
        ...
        Rect r1 = new Rect();
        Log.d("DragLayer", "rearrangementExists(): ignoreView:" + ignoreView.toString());
        for (View child: solution.map.keySet()) {
            Log.d("DragLayer", "rearrangementExists(): child:" + child.toString());
            if (child == ignoreView) continue;

            //Added by MC
            LauncherAppCardInfo item  = (LauncherAppCardInfo) child.getTag();
            if ((item != null) && item.mIsDummy) continue;
            CellAndSpan c = solution.map.get(child);
            Log.d("DragLayer", "rearrangementExists(): CellAndSpan:" + c.toString());
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            r1.set(c.x, c.y, c.x + c.spanX, c.y + c.spanY);
            if (Rect.intersects(r0, r1)) {
                if (!lp.canReorder) {
                    Log.d("DragLayer", "rearrangementExists() returns false HERE!");
                    return false;
                }
                mIntersectingViews.add(child);
                Log.d("DragLayer", "rearrangementExists()is adding the child to mIntersectingViews" + child.toString());
            }

            //Added by MC
            if (r1.contains(r0)){
                biggerView = child;
                Log.d("DragLayer", "rearrangementExists()found the bigger view:" +  biggerView.toString());
            }
        }
        ...
        for (View v : mIntersectingViews) {
            if (!addViewToTempLocation(v, mOccupiedRect, direction, solution)) {
                //Added by MC
                Log.d("DragLayer", "rearrangementExists() comes HERE HERE!");
                if (biggerView != null){
                    if (swapViews(biggerView, ignoreView, solution)) {
                        return true;
                    }
                }
                return false;
    ...
    ItemConfiguration simpleSwap(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX,
        ...
        Log.d("DragLayer", "simpleSwap() called findNearestArea() with pixelX:" + pixelX + " pixelY:" +
                pixelY + " spanX:" + spanX + " spanY: " + spanY + " and result:" + result[0] + " and " + result[1]);
        ...
        Log.d("DragLayer", "simpleSwap() called rearrangementExists() with the return value:" + success);
        if (!success) {
    ...
    private void copyCurrentStateToSolution(ItemConfiguration solution, boolean temp) {
        Log.d("DragLayer", "copyCurrentStateToSolution() called with the temp value:" + temp);
        int childCount = mShortcutsAndWidgets.getChildCount();
        for (int i = 0; i < childCount; i++) {
            View child = mShortcutsAndWidgets.getChildAt(i);
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            CellAndSpan c;
            Log.d("DragLayer", "LayoutParams:" + lp.toString() + " for the view:" + child.toString());
            if (temp) {
    ...
    */
    ...
    boolean createAreaForResize(int cellX, int cellY, int spanX, int spanY,
        ...
        Log.d("DragLayer", "mCellLayout.createAreaForResize() to be called with cellX:" + cellX
                + " cellY:"+ cellY + " spanX:" + spanX + " spanY:" + spanY);

        int[] pixelXY = new int[2];
        regionToCenterPoint(cellX, cellY, spanX, spanY, pixelXY);

        Log.d("DragLayer", "mCellLayout.createAreaForResize(): get the Center Pointer:"
                + pixelXY[0] +" " + pixelXY[1]);
