...
public class AppWidgetResizeFrame extends FrameLayout {
    ...
    public AppWidgetResizeFrame(Context context,
        ...
        if (itemInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_CARD){
            ...
            //TO-DO: limit resize mode base on position
            mMinHSpan = 1;
            mMinVSpan = 1;
        } else {
        ...
        Rect p;
        if (itemInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_CARD){
            p = LauncherAppWidgetHostView.getDefaultPaddingForCard(context, null);
        } else {
        ...
        Log.d("DragLayer", "new AppWidgetResizeFrame is created:" + this);
        Log.d("DragLayer", "and mWidgetView is marked as Unoccupied:" +mWidgetView);
    }
    ...
    private void resizeWidgetIfNeeded(boolean onDismiss) {
        Log.d("DragLayer", "resizeWidgetIfNeeded() for AppWidgetResizeFrame is called!!!!");
        ...
        Log.d("DragLayer", " onDismiss:" + onDismiss + " hSpanInc:"
                + hSpanInc + " and vSpanInc:" + vSpanInc);
        ...
        Log.d("DragLayer", " onDismiss:" + onDismiss + " vSpanDelta:"
                + vSpanDelta + " and hSpanDelta:" + hSpanDelta);
        ...
        if ((spanX ==1) && (spanY == 2)) return;
        if ((spanX >2) || (spanY > 2)) return;

        if (mCellLayout.createAreaForResize(cellX, cellY, spanX, spanY, mWidgetView,
                mDirectionVector, onDismiss)) {
            Log.d("DragLayer", "mCellLayout.createAreaForResize() returns TRUE!!!!");
            lp.tmpCellX = cellX;
            lp.tmpCellY = cellY;
            lp.cellHSpan = spanX;
            lp.cellVSpan = spanY;
            ...
            if (!onDismiss) {
                Log.d("DragLayer", "updateWidgetSizeRanges() is to be called!!!");
                updateWidgetSizeRanges(mWidgetView, mLauncher, spanX, spanY);
            }

            //Added by MC
            //If the size is changed, exit to re-fresh the display
            if (vSpanDelta != 0 || hSpanDelta != 0) {
                mWorkspace.exitWidgetResizeMode();
            }
        }
        Log.d("DragLayer", "mWidgetView.requestLayout() is to be called!!!");
        mWidgetView.requestLayout();
    }

    static void updateWidgetSizeRanges(AppWidgetHostView widgetView, Launcher launcher,
            int spanX, int spanY) {
        Log.d("DragLayer", "updateWidgetSizeRanges() for AppWidgetResizeFrame is called!!!!");
    ...
    public void onTouchUp() {
        Log.d("DragLayer", "onTouchUp() is called on the mCurrentResizeFrame" + this);
        int xThreshold = mCellLayout.getCellWidth() + mCellLayout.getWidthGap();
        int yThreshold = mCellLayout.getCellHeight() + mCellLayout.getHeightGap();
