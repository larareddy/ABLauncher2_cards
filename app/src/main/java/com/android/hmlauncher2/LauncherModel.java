...
import com.android.hmlauncher2.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class LauncherModel extends BroadcastReceiver {
    ...
    static final ArrayList<LauncherAppCardInfo> sBgAppCards =
            new ArrayList<LauncherAppCardInfo>();
    ...
    public interface Callbacks {
        ...
        public void bindAppCard(LauncherAppCardInfo info);
    ...
    public static final boolean[][] MEAL_CARD = new boolean[2][2];
    public static final boolean[][] SNACK_CARD = new boolean[1][2];
    public static final boolean[][] BITE_CARD = new boolean[1][1];
    public static final int MAX_MEAL_COUNT = 2;
    public static final int MAX_SNACK_COUNT = 4;
    public static final int MAX_BITE_COUNT = 4;
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                synchronized (sBgLock) {
                    ...
                    ItemInfo modelItem = sBgItemsIdMap.get(itemId);
                    // added by fca demo: our dummy card item id always be -1
                    if (modelItem != null) {
    ...
    static void updateItemInDatabase(Context context, final ItemInfo item) {
        ...
        if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_CARD) {
            updateDummyAppCard(null);
        }
                ...
                synchronized (sBgLock) {
                    ...
                    switch (item.itemType) {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                            ...
                            break;
                        case LauncherSettings.Favorites.ITEM_TYPE_CARD:
                            sBgAppCards.add((LauncherAppCardInfo) item);
                            break;
                    }
                }
            }
    ...
    static void updateLayoutCellSize(int width, int height) {
        sCellWidth = width;
        sCellHeight = height;
    }
                ...
                synchronized (sBgLock) {
                    switch (item.itemType) {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                            ...
                            break;
                        case LauncherSettings.Favorites.ITEM_TYPE_CARD:
                            sBgAppCards.remove(item);
                            updateDummyAppCard((LauncherAppCardInfo)item);
                    }
    ...
    private class LoaderTask implements Runnable {
        ...
        private void loadWorkspace() {
            ...
            synchronized (sBgLock) {
                ...
                try {
                    ...
                    LauncherAppCardInfo appCardInfo;
                    int container;
                    long id;
                    Intent intent;
                    UserHandle user;
                    ...
                    while (!mStopped && c.moveToNext()) {
                        try {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                ...
                                break;
                            case LauncherSettings.Favorites.ITEM_TYPE_CARD:
                                int cardId = c.getInt(appWidgetIdIndex);
                                id = c.getLong(idIndex);
                                appCardInfo = new LauncherAppCardInfo(cardId,
                                            "test");
                                appCardInfo.id = id;
                                appCardInfo.screen = c.getInt(screenIndex);
                                appCardInfo.cellX = c.getInt(cellXIndex);
                                appCardInfo.cellY = c.getInt(cellYIndex);
                                appCardInfo.spanX = c.getInt(spanXIndex);
                                appCardInfo.spanY = c.getInt(spanYIndex);
                                ...
                                if (container != LauncherSettings.Favorites.CONTAINER_DESKTOP &&
                                        container != LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                                    Log.e(TAG, "Card found where container != " +
                                            "CONTAINER_DESKTOP nor CONTAINER_HOTSEAT - ignoring!");
                                    continue;
                                }
                                appCardInfo.container = c.getInt(containerIndex);

                                // check & update map of what's occupied
                                if (!checkItemPlacement(occupied, appCardInfo)) {
                                    break;
                                }
                                sBgItemsIdMap.put(appCardInfo.id, appCardInfo);
                                sBgAppCards.add(appCardInfo);
                                break;
                            }
                        } catch (Exception e) {
        ...
        private void bindWorkspaceItems(final Callbacks oldCallbacks,
                ...
                final ArrayList<LauncherAppCardInfo> appCards,
            ...
            if (appCards != null) {
                // Bind the cards, one at a time
                N = appCards.size();
                for (int i = 0; i < N; i++) {
                    final LauncherAppCardInfo card = appCards.get(i);
                    final Runnable r = new Runnable() {
                        public void run() {
                            Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                            if (callbacks != null) {
                                callbacks.bindAppCard(card);
                            }
                        }
                    };
                    if (postOnMainThread) {
        ...
        private void bindWorkspace(int synchronizeBindPage) {
            ...
            ArrayList<LauncherAppWidgetInfo> appWidgets =
                    new ArrayList<LauncherAppWidgetInfo>();
            ArrayList<LauncherAppCardInfo> appCards =
                    new ArrayList<LauncherAppCardInfo>();
            ...
            bindWorkspaceItems(oldCallbacks, currentWorkspaceItems, currentAppWidgets, appCards,
                    currentFolders, null);
            ...
            mDeferredBindRunnables.clear();
            bindWorkspaceItems(oldCallbacks, otherWorkspaceItems, otherAppWidgets, null, otherFolders,
    ...
    private static void markCells(int cellX, int cellY, int spanX, int spanY, boolean[][] cells,
                                  boolean value) {
        final int countX = cells[0].length;
        final int countY = cells.length;
        if (cellX < 0 || cellY < 0) return;
        for (int y = cellY; y < cellY + spanY && y < countY; y++) {
            for (int x = cellX; x < cellX + spanX && x < countX; x++) {
                cells[y][x] = value;
            }
        }
    }

    private static LauncherAppCardInfo generateDummyAppCard(int cellX, int cellY, int spanX, int spanY) {
        LauncherAppCardInfo dummyCard =  new LauncherAppCardInfo(-1,
                "test");
        dummyCard.mIsDummy = true;
        dummyCard.id = -1;
        dummyCard.screen = 0;
        dummyCard.cellX = cellX;
        dummyCard.cellY = cellY;
        dummyCard.spanX = spanX;
        dummyCard.spanY = spanY;
        dummyCard.minSpanX = 1;
        dummyCard.minSpanY = 1;
        dummyCard.container = LauncherSettings.Favorites.CONTAINER_DESKTOP;
        return dummyCard;
    }

    private static LauncherAppCardInfo findAndGenerateDummyCard(boolean[][] cells, boolean[][] compareTo) {
        final int countX = cells[0].length;
        final int countY = cells.length;
        for (int y = 0; y < countY; y ++) {
            for (int x = 0; x < countX; x ++) {
                if (!cells[y][x]) {
                    final int expectedCountX = compareTo[0].length;
                    final int expectedCountY = compareTo.length;
                    boolean occupied = false;
                    for (int cy = 0; cy < expectedCountY; cy ++) {
                        if ((y + cy) > (countY - 1)) {
                            occupied = true;
                            break;
                        }
                        for (int cx = 0; cx < expectedCountX; cx ++) {
                            if ((x + cx) > (countX - 1)) {
                                occupied = true;
                                break;
                            }
                    ...
                    if (!occupied) {
                        return generateDummyAppCard(x, y, expectedCountX, expectedCountY);
                    }
                }
            }
        }
    ...
    public static ArrayList<LauncherAppCardInfo> generateDummyAppCards(LauncherAppCardInfo inheritFrom) {
        ArrayList<LauncherAppCardInfo> dummyCards = new ArrayList<LauncherAppCardInfo>();
        final int countX = getCellCountX();
        final int countY = getCellCountY();
        boolean[][] cells = new boolean[countY][countX];
        int countMeal = 0;
        int countSnack = 0;
        int countBite = 0;
        for (LauncherAppCardInfo cardInfo : sBgAppCards) {
            markCells(cardInfo.cellX, cardInfo.cellY, cardInfo.spanX, cardInfo.spanY, cells, true);
        ...
        LauncherAppCardInfo dummyCard = null;
        if (inheritFrom != null) {
            dummyCards.add(generateDummyAppCard(inheritFrom.cellX, inheritFrom.cellY, inheritFrom.spanX, inheritFrom.spanY));
            markCells(inheritFrom.cellX, inheritFrom.cellY, inheritFrom.spanX, inheritFrom.spanY, cells, true);
            switch (inheritFrom.getCardType()) {
                case CardsProvider.TYPE_MEAL:
                    countMeal ++;
                    break;
                case CardsProvider.TYPE_BITE:
        ...
        for (int i = countMeal; i < MAX_MEAL_COUNT; i ++) {
            dummyCard = findAndGenerateDummyCard(cells, MEAL_CARD);
            if (dummyCard != null) {
                dummyCards.add(dummyCard);
                markCells(dummyCard.cellX, dummyCard.cellY, dummyCard.spanX, dummyCard.spanY, cells, true);
            } else {
                break;
            }
        }

        for (int i = countSnack; i < MAX_SNACK_COUNT; i ++) {
            dummyCard = findAndGenerateDummyCard(cells, SNACK_CARD);
            if (dummyCard != null) {
                dummyCards.add(dummyCard);
                markCells(dummyCard.cellX, dummyCard.cellY, dummyCard.spanX, dummyCard.spanY, cells, true);
            } else {
        ...
        for (int i = countBite; i < MAX_BITE_COUNT; i ++) {
            dummyCard = findAndGenerateDummyCard(cells, BITE_CARD);
            if (dummyCard != null) {
                dummyCards.add(dummyCard);
                markCells(dummyCard.cellX, dummyCard.cellY, dummyCard.spanX, dummyCard.spanY, cells, true);
            } else {
    ...
    public static void updateDummyAppCard(LauncherAppCardInfo inheritFrom) {
        final ArrayList<LauncherAppCardInfo> dummyCards = generateDummyAppCards(inheritFrom);
        Runnable r = new Runnable() {
            public void run() {
                if (sLauncherModel.mCallbacks != null) {
                    Callbacks callbacks = sLauncherModel.mCallbacks.get();
                    if (callbacks != null) {
                        callbacks.bindDummyAppCard(dummyCards);
                    } else {
                        Log.i(TAG, "updateDummyAppCard: mCallbacks get() return null");
                    }
                } else {
                    Log.i(TAG, "updateDummyAppCard: mCallbacks null");
                }
            }
        };
        sLauncherModel.runOnMainThread(r, MAIN_THREAD_BINDING_RUNNABLE);
    }
}
